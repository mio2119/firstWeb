...
1. **Codex 数据/后端轨**：负责后端、JSON、数据类型、QA 模板、脚本、工作台和校验规则。
2. **Claude 前端轨**：负责现有前端页面和组件中的最小展示增强。

Claude Code 的默认任务不是补 JSON，也不是改后端。Claude 如果发现缺字段或缺 demo 数据，应先写出字段需求或数据需求，交给 Codex 数据/后端轨处理。

本次允许做：

1. Codex 增加少量静态 demo 数据。
2. Codex 扩展 admissions 数据类型。
3. Claude 在现有页面和组件中增加解释型内容区块。
4. Claude 增加必要的小组件，但必须沿用现有视觉风格。
5. Codex 补充 QA 模板，让 AI 顾问能回答“专业黑盒、推荐理由、路径风险”等 demo 问题。
...
5. 不把模拟数据包装成权威真实结果。
6. 不让 Claude 越界修改 `public/data/**`、`backend/**` 或 `src/data/types/**`，除非用户明确临时授权。
...
### Codex-owned Data / Backend Scope
...
Claude 可以读取 Codex 产出的 JSON/types，但不要直接修改它们。如果 Claude 认为前端需要新字段，应记录字段需求。
...
### Slice 2: Guangdong Demo Data (Codex)
...
### Slice 3: Detail Panel Explanation (Claude)
...
### Slice 4: User Profile Signal (Claude)
...
### Slice 5: QA Demo Templates (Codex)
...
只修失败项。如果失败项属于数据、类型、后端或 JSON，由 Codex 修；如果失败项属于前端展示或视觉白名单，由 Claude 修。不要互相回滚对方的改动。
...
9. Claude 未修改 Codex-owned 数据/后端文件；Codex 未替 Claude 重做前端视觉。
