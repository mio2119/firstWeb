...
const AddToPlanButton: React.FC<AddToPlanButtonProps> = ({ university, className = "" }) => {
    const { addToPlan, removeFromPlan, updatePlanStrategy, isInPlan, getPlanItem } = usePlan();
    ...
    const handleStrategyChange = (strategy: StrategyType) => {
        updatePlanStrategy(university.id, strategy);
        setIsOpen(false);
    };
                    ...
                    >
                         {!isSaved ? (
                             ...
                             <div className="flex flex-col gap-1">
                                 ...
                                 <button onClick={() => handleStrategyChange('rush')} className="flex items-center gap-3 w-full px-3 py-2 rounded-lg hover:bg-rose-50 text-left group transition-colors">
                                     <div className="p-1.5 rounded-md bg-rose-100 text-rose-600"><Zap className="w-3 h-3" /></div>
                                     <span className="text-sm font-bold text-slate-700 group-hover:text-rose-700">改为冲刺</span>
                                 </button>
                                 <button onClick={() => handleStrategyChange('stable')} className="flex items-center gap-3 w-full px-3 py-2 rounded-lg hover:bg-amber-50 text-left group transition-colors">
                                     <div className="p-1.5 rounded-md bg-amber-100 text-amber-600"><Shield className="w-3 h-3" /></div>
                                     <span className="text-sm font-bold text-slate-700 group-hover:text-amber-700">改为稳妥</span>
                                 </button>
                                 <button onClick={() => handleStrategyChange('safe')} className="flex items-center gap-3 w-full px-3 py-2 rounded-lg hover:bg-emerald-50 text-left group transition-colors">
                                     <div className="p-1.5 rounded-md bg-emerald-100 text-emerald-600"><Anchor className="w-3 h-3" /></div>
                                     <span className="text-sm font-bold text-slate-700 group-hover:text-emerald-700">改为保底</span>
                                 </button>
